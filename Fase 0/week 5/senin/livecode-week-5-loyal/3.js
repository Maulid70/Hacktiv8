...
function transferWindowFilter(scoutList, playerList) {
  ...
  for(i = 0; i < scoutList.length; i++){
    var priceRange = scoutList[i].priceRange.split('-')
    var kelas = {
      pos : '',
      priceRange : 0,
      suggestedPlayer : []
    }
    kelas.pos = scoutList[i].pos
    kelas.priceRange = scoutList[i].priceRange
    for(j = 0; j < playerList.length; j++){
      if(kelas.pos === playerList[j].position){
        if(playerList[j].price >= priceRange[0] && playerList[j].price <= priceRange[1]){
          kelas.suggestedPlayer.push(playerList[j].name)
        }
      }
    }
    // kelas.suggestedPlayer = playerList[j]
    hasil.push(kelas)
  }
  return hasil
}
